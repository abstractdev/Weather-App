...
export function displayWeatherData(weatherData) {
    ...
    getEle('.temp').textContent = `Temperature: ${Math.round(weatherData.temperature).toString()}°F`;
    getEle('.feels').textContent = `Feels like: ${Math.round(weatherData.feelsLike).toString()}°F`;
    getEle('.condition').textContent = weatherData.condition;
    getEle('.humidity').textContent = `Humidity: ${weatherData.humidity.toString()}%`;
    ...
    const celsiusListener = (() => {
        cButton.addEventListener("click", () => {
            getEle('.temp').textContent = `Temperature: ${weatherData.temperatureC.toFixed(2).toString()}°C`;
            getEle('.feels').textContent = `Feels like: ${weatherData.feelsLikeC.toFixed(2).toString()}°C`;
          });
    })()
